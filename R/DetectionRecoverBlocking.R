...
DetectionRecoverBlocking = function(z, h, k, k1, block_size=64) {
  ...
  for (i in seq(1, 1+(n_blocks_height-1)*non_overlap_height, length.out = n_blocks_height)) {
    for (j in seq(1, 1+(n_blocks_width-1)*non_overlap_width, length.out = n_blocks_width)) {
      ...
      if (i == 1 & j == 1){
        cat("Block # ",p,":Block width from", j, "to", (j + block_size - 1),
        ...
        result_matrix[j:(j + block_size - 1 - overlap_width/2),
                      i:(i + block_size - 1 - overlap_height/2)] =
          edge_temp[(1+(kk + k)):(block_size+(kk + k)),
                    (1+(kk + k)):(block_size+(kk + k))][1:(1 + block_size - 1 - overlap_width/2),
                                                        1:(1 + block_size - 1 - overlap_height/2)]
        #record final recovery matrix
        recover_matrix[j:(j + block_size - 1 - overlap_width/2),
                       i:(i + block_size - 1 - overlap_height/2)] =
          RecoverSurface(n_pad, k1, block_pad, edge_temp)[(1+(kk + k)):(block_size+(kk + k)),
                                                          (1+(kk + k)):(block_size+(kk + k))][1:(1 + block_size - 1 - overlap_width/2),
                                                                                              1:(1 + block_size - 1 - overlap_height/2)]
      } else if (j == 1) {
        cat("Block # ",p,":Block width from", j, "to", (j + block_size - 1),
        ...
        edge_temp = EdgeDectect(block_pad, k, n_pad, ls_beta1, ls_beta2, 0.05)
        #record final edge matrix
        result_matrix[(j + overlap_width/2):(j + block_size - 1 - overlap_width/2),
                      i:(i + block_size - 1 - overlap_height/2)] =
          edge_temp[(1+(kk + k)):(block_size+(kk + k)),
                    (1+(kk + k)):(block_size+(kk + k))][(1 + overlap_width/2):(1 + block_size - 1 - overlap_width/2),
                                                        1:(1 + block_size - 1 - overlap_height/2)]
        #record final recovery matrix
        recover_matrix[(j + overlap_width/2):(j + block_size - 1 - overlap_width/2),
                       i:(i + block_size - 1 - overlap_height/2)] =
          RecoverSurface(n_pad, k1, block_pad, edge_temp)[(1+(kk + k)):(block_size+(kk + k)),
                                                          (1+(kk + k)):(block_size+(kk + k))][(1 + overlap_width/2):(1 + block_size - 1 - overlap_width/2),
                                                                                              1:(1 + block_size - 1 - overlap_height/2)]
    ...
    p = p + 1
    if (i == 1) {
      cat("Block # ",p,":Horizontally last block width from", width - block_size + 1, "to", width,
          "height from", i, "to", (i + block_size - 1), "\n")
      ...
      edge_temp = EdgeDectect(block_pad, k, n_pad, ls_beta1, ls_beta2, 0.05)
      #record final edge matrix
      result_matrix[(width - block_size + overlap_width/2 + 1):width,
                    i:(i + block_size - 1 - overlap_height/2)] =
        edge_temp[(1+(kk + k)):(block_size+(kk + k)),
                  (1+(kk + k)):(block_size+(kk + k))][(1 + overlap_width/2):block_size,
                                                      i:(i + block_size - 1 - overlap_height/2)]
      #record final recovery matrix
      recover_matrix[(width - block_size + overlap_width/2 + 1):width,
                     i:(i + block_size - 1 - overlap_height/2)] =
        RecoverSurface(n_pad, k1, block_pad, edge_temp)[(1+(kk + k)):(block_size+(kk + k)),
                                                        (1+(kk + k)):(block_size+(kk + k))][(1 + overlap_width/2):block_size,
                                                                                            i:(i + block_size - 1 - overlap_height/2)]
  ...
  for (j in seq(1, 1+(n_blocks_width-1)*non_overlap_width, length.out = n_blocks_width)) {
    p = p + 1
    if (j == 1) {
      cat("Block # ",p,":Vertically last block width from", j, "to", j + block_size - 1,
